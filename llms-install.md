...
Use the pinned `cite-nexus-mcp==0.2.2` package from PyPI.

1. Check Python 3.11+ and uv are available.
2. Run `uvx --from cite-nexus-mcp==0.2.2 cite-nexus-mcp --version`.
3. Run `uvx --from cite-nexus-mcp==0.2.2 cite-nexus-mcp --no-env-file --list-providers`.
4. Configure an MCP stdio connection with executable `uvx` and separate arguments
   `--from`, `cite-nexus-mcp==0.2.2`, `cite-nexus-mcp`, `--no-env-file`.
